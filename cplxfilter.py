...
<<<<<<< HEAD
    ...
    if not hasattr(required_fsiz, "__len__"):
        fsize = np.ones(3) * required_fsiz
    else:
        if len(required_fsiz) != 3:
...
<<<<<<< HEAD
    if np.mod(np.array(fsize), 2).any():
        ...
        yy = np.array(range(-int(half_fsiz[1]), int(half_fsiz[1]) + 1))
        zz = np.array(range(-int(half_fsiz[2]), int(half_fsiz[2]) + 1))
    else:
        half_fsiz = (np.array(filtersize) - 1) / 2.0
