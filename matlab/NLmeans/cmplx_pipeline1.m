function [denoised_img,swi_n2,swi_n1,swi_nOrig,swi_n0,sigma_real,sigma_imag] = cmplx_pipeline1(mag,phase,NLfilter,searcharea,patchsize,rician,hfactor,noisepc)
...
% add some noise
if nargin <8 || isempty(noisepc) || noisepc <= 0
    ...
    real_img=real_img-mode(real_img(:));imag_img=imag_img-mode(imag_img(:));
...
%ima=t2swi;
  ...
  % Denoise
    ...
    display (['Imag Rescaled mode ' numtr(mode(MRIdenoisedImagRescaled(:)))])
