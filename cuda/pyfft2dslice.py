...
for slab in xrange(0, 4):
    ...
    %time im1_ft_tmp = im1_gpu.get()
    im1_ft[:, :, slab * mid:mid * (slab + 1)] = im1_ft_tmp
    del im1_gpu
im2_ft = im2.copy()
for slab in xrange(0, 4):
    %time im2_gpu = gpuarray.to_gpu(im2[:, :, slab * mid:mid * (slab + 1)].astype(numpy.complex64))
    %time plan.execute(im2_gpu)
    %time im2_ft_tmp = im2_gpu.get()
    im2_ft[:, :, slab * mid:mid * (slab + 1)] = im2_ft_tmp
    del im2_gpu
...
for slab in xrange(0, 4):
    ...
    %time corr_ft_tmp = conv_gpu.get()
    corr_gpu[:, :, slab * mid:mid * (slab + 1)] = corr_ft_tmp
    del conv_gpu
