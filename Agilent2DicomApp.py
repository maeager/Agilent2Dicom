...
class Agilent2DicomWindow(QtGui.QWidget):
    ...
    def ChangeFDFDicomPath(self):
        try:
            newdir = str(
                QFileDialog.getExistingDirectory(self, "Select DICOM Directory"))
...
#                    event.ignore()
    ...
    def ChangeFIDDicomPath(self):
        try:
            newdir = str(QFileDialog.getExistingDirectory(
                self, "Select or create new DICOM Directory"))
            files = os.listdir(newdir)
            dcmfiles = [f for f in files if f.endswith('.dcm')]
            if len(dcmfiles) == 0:
                print 'FDF output DICOM folder does not contain any dcm files'
    ...
    def GetDarisID(self, inpath):
        ...
        try:
            ...
            procpar, pep = ReadProcpar.ReadProcpar(
    ...
    def Send2Daris(self):
        try:
            ...
            tmp_msg = "Sending DICOM directory " + \
                str(dicom_dir) + " to DaRIS.\n" + cmd + "\n\nAre you sure?"
            reply = QtGui.QMessageBox.question(self, 'Message',
    ...
    def getCommandArgs(self):
        ...
        if self.ui.checkBox_gaussian.isChecked():
            argstr += ' -g %s -j %s' % (
                str(self.ui.lineEdit_gsigma.text()), str(self.ui.lineEdit_gorder.text()))
        ...
        if self.ui.checkBox_wiener.isChecked():
            argstr += ' -w %s -z %s' % (str(self.ui.lineEdit_wiener_size.text()), str(
                self.ui.lineEdit_wiener_noise.text()))
        return argstr
    ...
    def CheckFID(*args):  # send_button):
        try:
            ...
            print 'check path: %s' % thispath
            cmd1 = os.path.join(
                thispath, 'dcheck.sh') + ' -o ' + str(output_dir)
            print(cmd1)
            cmd = cmd_header + cmd1 + ')'
            print(cmd)
            os.system(cmd)
